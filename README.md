...
先把真实凭证写入 Gateway 主机的 `~/.openclaw/.env`，不要提交到 GitHub 或发送到飞书，然后执行 `python3 openclaw/install.py --install-deps --configure-mcp`。安装器会把旧版本备份到 `~/.openclaw/skill-backups/amazon-sellersprite-research/`，安装中途失败时自动恢复。
...
- **V1 企划闭环**：意向产品 → 市场分析 → 竞品分析与优化策略 → SWOT → ABA 人工维护页 → MCP原始数据 → 数据来源。
- **统一证据对象**：先把竞品、类目、月份、市场、关键词、VOC 和来源归一到 `evidence.json`，通过 Python 质量门禁后再同时生成 Markdown 与 Excel，避免不同 Sheet 使用不同口径。
- **查询口径锁定**：采集前固定分析月份、最近完整月份、连续 12 个月、Top N、新品定义和目标类目节点；当前未完整月份只作快照。
- **竞品 Sheet 标准化**：命令中提供 3 个以分号分隔的代表竞品 ASIN；通过 `asin_detail` 校验后，系统仅在必要时补充 1 个相关竞品，最终固定为 4 个样本。Sheet 采用“基础参数对比 + 差异化信号对比 + 四张纵向竞品卡片”结构；截图指标标为快照，实时 MCP 字段优先。无合格补充样本时明确标注 `N/A` 与数据缺口。
- **证据驱动**：企划结论必须来自 SellerSprite MCP、Amazon 前端、已有研究报告、用户输入或明确标注的外部补充数据。
- **渲染前门禁**：`validate_product_planning_evidence.py` 检查类目覆盖、12 月趋势、关键词覆盖、VOC 样本、证据引用、Sheet 顺序、模板占位和敏感信息；`FAIL` 时不生成正式报告。
- **会议版 Excel**：默认不输出 `成本试算` 和 `销售计划`；如需财务测算，必须由用户提供成本、物流、售价、广告占比等假设后单独追加。
- **默认数据驱动模板**：优先使用 `templates/product_planning_standard_template_V1_data_driven.xlsx`；模板仅保留七 Sheet 结构、样式、图表和字段位置，运行时由已验证证据填充。功能一至四为已验证卖点，功能五至七为未满足需求；美化版与旧 V1 工作簿作为兼容备份。
...
```bash
python scripts/python/sellersprite_skill_tools.py check-runtime-config --strict
```

如果 Windows 没有把 Python 加入 PATH，可使用实际解释器路径，例如：

```text
%LOCALAPPDATA%\Python\pythoncore-3.14-64\python.exe scripts/python/sellersprite_skill_tools.py check-runtime-config --strict
```
...
```bash
python scripts/python/sellersprite_skill_tools.py install-skill --target claude
```

Codex 风格：

```bash
python scripts/python/sellersprite_skill_tools.py install-skill --target codex
```

只安装单个技能：

```bash
python scripts/python/sellersprite_skill_tools.py install-skill --target codex --skill amazon-analyse
```
...
新版统一证据与质量门禁流程见 [references/product-planning-integrated-workflow.zh.md](references/product-planning-integrated-workflow.zh.md)。
